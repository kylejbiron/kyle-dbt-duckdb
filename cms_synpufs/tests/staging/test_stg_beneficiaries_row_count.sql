...
WITH actual_record_count AS (
    SELECT COUNT(*) AS record_count FROM {{ ref('stg_beneficiaries') }}
),
...
expected_record_count AS (
    ...
    SELECT SUM(record_count) AS expected_record_count FROM record_counts
)
...
WHERE actual_record_count.record_count != expected_record_count.expected_record_count
